import { Sequencer, type NoteEvent } from './pattern'
import './style.css'

let song1: NoteEvent[]
function init_others() {

  song1 = Sequencer.parse(`
c2 - c3 c2 | a2 - - d2 | c2 - - - |
`.repeat(100))
}
    ...
    bass: {
      envelope: {
        ...
        pitch: {
          ...
          decayTimeInSeconds: 0.02,
          sustainLevel: 0,
          releaseTimeInSeconds: 0.0
        },
      }
    }
  })
...
function init_keyboard() {
  ...
  document.addEventListener('keydown', async (e) => {
    ...
    if (e.key === ' ') {
      if (synth_worklet) {
        sequencer.play(synth_worklet, 'bass', song1, 360)
      }
    }
  })
}
...
function drawPianoGrid(width: number, height: number) {
  ...
  sequencer.activeNotes.forEach(freq => {
